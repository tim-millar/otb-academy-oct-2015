...
RSpec.describe "methods in ruby" do
  ...
  it "is not possible to call methods with the wrong number of arguments" do
    expect { a_global_method }.to raise_error( ArgumentError, /arguments/ ) # HINT: Fill in the error and part of the message

    expect { a_global_method(1,2,3) }.to raise_error( ArgumentError, /arguments/ ) # HINT: Fill in the error and part of the message
  end
  ...
  context "methods on objects" do
    ...
    it "raises an error when calling a private method" do
      ...
      expect { an_object.the_private_method }.to raise_error( NoMethodError, /method/ )
    end
