...
def corner_case_ xs    # possible problem here
...
RSpec.describe "Harry Potter prices helper functions" do
  ...
  it "correctly identifies the corner case" do
    expect(corner_case?([3,3,3,3,3])).to eq(false)
    expect(corner_case?([2,2,2,2,3])).to eq(false)
    expect(corner_case?([4,3,5,3,3])).to eq(false)
  end
end
